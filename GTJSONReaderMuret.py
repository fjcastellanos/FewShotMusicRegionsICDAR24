...
class GTPage:
    ...
    def getBBoxPageAndSymbols(self, considered_classes):
        self.__i_integrity()
        ...
        for region in self.regions:
            if considered_classes is None or region.isNameInList(considered_classes):
                coords_regions = (region.coord_p1[0], region.coord_p1[1], region.coord_p2[0], region.coord_p2[1])
                symbols = [str(s) for s in region.symbols]
                list_bbox.append((coords_regions, symbols))
    ...
    def addGT(self, gt_im, considered_classes, vertical_reduction_regions=0., nb_annotated_regions=None):
        self.__i_integrity()
        
        nb_annotated_regions_added = 0
        ...
        for region in self.regions:
            if region.name_label in considered_classes:
                ...
                if nb_annotated_regions != -1 and nb_annotated_regions_added >= nb_annotated_regions:
                    if gt_im is not None:
                        gt_im[x_start:x_end, y_start:y_end] = 2 #not annotated region but included as reference
                else:
                    if gt_im is not None:
                        gt_im[x_start:x_end, y_start:y_end] = 1 #Annotated region
                    nb_annotated_regions_added+=1

        return nb_annotated_regions_added
class GTJSONReaderMuret:
    ...
    def getListBoundingBoxesAndSymbols(self, considered_classes=None):
        self.__i_integrity()
        list_bbox = []

        for page in self.pages:
            list_bbox_page_with_symbols = page.getBBoxPageAndSymbols(considered_classes)
            for bbox_page in list_bbox_page_with_symbols:
    ...
    def generateGT(self, considered_classes, img_shape, vertical_reduction_regions = 0., nb_annotated_regions=-1):
        assert(type(considered_classes) is list)
        assert(type(img_shape) is tuple)
        assert(len(img_shape) == 2)

        gt_im = np.zeros(img_shape)
        count_number_added_regions = 0
        for page in self.pages:
            if nb_annotated_regions != -1:
                number_regions_to_be_added_in_page = nb_annotated_regions - count_number_added_regions
            else:
                number_regions_to_be_added_in_page = -1
            number_added_regions = page.addGT(gt_im, considered_classes, vertical_reduction_regions, number_regions_to_be_added_in_page)
            count_number_added_regions += number_added_regions

        return gt_im, count_number_added_regions

    def get_number_annotated_regions(self, considered_classes, nb_annotated_regions):
        assert(type(considered_classes) is list)
        ...
        for page in self.pages:
            if nb_annotated_regions != -1:
                ...
                number_regions_to_be_added_in_page = -1
            number_added_regions = page.addGT(None, considered_classes, 0, number_regions_to_be_added_in_page)
            count_number_added_regions += number_added_regions

        return count_number_added_regions
