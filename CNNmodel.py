...
def get_model(input_size, no_mask, nb_layers, nb_filters, k_size, dropout=0.2, stride=1):
    ...
    #opt = tf.keras.optimizers.SGD(learning_rate=lr_schedule)     # unet
    opt = tf.keras.optimizers.SGD(lr=0.01, decay=1e-6, momentum=0.9, nesterov=False)
...
def train(model, path_out_model, train_generator, val_generator, steps_per_epoch, steps_per_epoch_val, nb_val_pages, batch_size, preepochs = 0, epochs=200, patience=20):
